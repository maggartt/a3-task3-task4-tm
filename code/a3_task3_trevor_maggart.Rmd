...
Read in the urchins data & convert to 'lower_case_snake'
...
ggplot(data = purple_count,
  ...
  theme_grey() +
  scale_color_manual(values = c("#ff9999", "#ffca89", "#fffa78", "#bdf2a7", "#7bead6")) +
...
ggplot(data = purple_count, 
       aes(x = year, y = count)) +
  geom_col(aes(fill = site_full, color = site_full)) +
  theme_grey() +
  facet_wrap(~site_full,
             scales = 'free_x') + # Facet wrap by 'site_full' 
  scale_color_manual(values = c("white", "white", "white", "white", "white")) +
  scale_fill_manual(values = c("#ff9999", "#ffca89", "#fffa78", "#bdf2a7", "#7bead6")) +
  labs(title = "Distribution of Purple Urchins from 2008-2020",
       x = "Year",
       y = "Total Number of Purple Urchins",
       key = "Site",
