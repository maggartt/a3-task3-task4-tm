...
#### Compare the Control and Annual treatments with a histogram

```{r}
ggplot(data = purple_urchins_NAPL_2010,
       aes(x = size)) +
  geom_histogram(aes(fill = treatment, color = treatment),bins=10) +
  scale_color_manual(values = c("white","white")) +
  facet_wrap(~ treatment) +
  theme_gray() +
  labs(x = "Size (cm)",
       y = "Count",
       caption = "Histogram comparing the size distribution of the different treatment groups") +
  theme(legend.position = "none",
        plot.caption = element_text(hjust = 0.5),
)
```

#### Compare the Control and Annual treatments with a QQplot
...
ggplot(data = purple_urchins_NAPL_2010, aes(sample=size)) +
  ...
  labs(x = "Theoretical Quantiles",
       y = "Size (cm)",
       caption = "QQ-plot comparing the size distribution of the different treatment groups") +
  theme(legend.position = "none",
        plot.caption = element_text(hjust = 0.5),
)
```

We can see by analyzing the graphs that both the treatment groups, annual and control, are positively skewed. This trend is more apparent in the annual treatment, but it is still slightly evident in control treatment as well. In addition, both plots show that our data distribution is not normally distributed. This is apparent in the QQplot as the stat line does not match the tail ends of the distribution. 
...
purple_urchins_NAPL_2010_summary <- purple_urchins_NAPL_2010 %>%
  ...
  summarize(mean_size = mean(size, na.rm = TRUE), # Create mean column
            median_size = median(size, na.rm = TRUE), # Create median column
            sd_size = sd(size, na.rm = TRUE), # Create standard deviation column
...
This table supports what the graphs display. There is not a substantial difference in mean, median, nor standard deviation. However, to truly know significance, we should conduct additional analyses in future work. 
